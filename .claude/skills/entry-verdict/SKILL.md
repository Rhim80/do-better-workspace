...
# entry-verdict — 새 자리 판정 (Go / 조건부 / 안됨)

> **한 줄**: "이 자리 들어갈까?" — 5-Layer 점수 + 월매출 잠재력 + BEP 임대 vs 시세 + 7개 핵심 질문으로 한 페이지 판정.
> **언제 씀**: 신규 출점·이전·확장 후보지를 평가할 때. 기존 매장 진단은 `zone-diagnosis`.
> **출력**: `50-resources/data/verdict-{zone_name}-{YYYYMMDD}.md` + HTML 한 페이지 카드
...
**이게 뭘 해주나요?**
새 자리 후보를 평가해서 들어갈지 말지 한 페이지 결론을 뽑아줍니다. **Go(들어가도 됨) / Conditional(조건부) / No-Go(안됨)**.

**이럴 때 쓰세요**
- "2호점 후보 3곳 중 어디?"
- "이 자리 임대료가 이만큼인데 매출 잠재력이 그만한가?"
- "직관으로는 좋은데 데이터로 한 번 검증해보고 싶다"
- "이 자리 BEP(손익분기) 임대가 얼마인가? 시세랑 차이는?"

**결과로 손에 쥐는 것**
- **Verdict**: Go / Conditional / No-Go
- **5-Layer 점수** (0-10점): 수요(Volume·Rhythm) · 경쟁(Pressure) · 콘셉트(Concept)
- **Magnitude**: 월매출 잠재력 보수·중립·낙관 + BEP 임대 vs 실제 시세 + Δ%
- **7 Critical Questions**: 7개 핵심 질문 충족·미충족·보류
- **재검토 조건**: Conditional일 때 어떤 조건에서 다시 봐야 하는지
- **현장 답사 체크리스트**: 데이터로 확인 못 한 항목

**주의**
- 자리 결정자가 아니라 의사결정 보조 도구. **마지막 판단은 사람.**
- Magnitude는 모델링 추정치. 실제 매출은 운영·메뉴·마케팅 영향이 더 큼.
...
→ 자동으로 비교 매트릭스 생성 (`50-resources/data/verdict-comparison-{YYYYMMDD}.md`):
- 5-Layer 점수 나란히 비교
- Magnitude 비교 표
- 7 CQ 매트릭스
- 추천 후보 + 핵심 리스크
