...
> `cp participants/김재균-뜨락/CLAUDE.md ./CLAUDE.md`
...
- **브랜드**: 뜨락 (한식, 청담동)
- **카테고리**: 한식
- **사업자**: 120-01-58365 (공동사업자: 신숙희·신현숙)
- **개업**: 1998-05-04 (28주년)
- **본점 주소**: 서울 강남구 영동대로142길 13-3 (청담동)
- **법인 수**: 3+ (난로회 공통 프로필)
...
- 객단가:
- 운영시간:
- 주요 페르소나 (청담동 결: 회식·접대·프리미엄):
- 향후 출점 후보 상권:

## 도메인 결 — 청담 한식

- 청담동 = 객단가·프리미엄 결, 다이닝·양식·일식 P94+ 강세 상권
- 회식·접대 페르소나(P02) 가중치 높음 — 룸·프라이빗·주류 매출
- 객단가 P30~P75 적정 위치 검증 중요 (concept-fit 핵심)
...
1. `store-interview` — **내 매장 인터뷰** (AI가 단계별로 묻고 personal-context.md 자동 작성)
   - 청담 한식 결: 회식·접대 페르소나(P02) 비중·룸 운영·VIP 단골 결 강조
2. `zone-diagnosis 압구정동` 또는 본인 동 — **내 자리 진단** (지금 상권 어떤 상태?)
...
4. 보조 스킬 (entry-verdict가 자동 호출):
   ...
   - `concept-fit` — **객단가·콘셉트 적합도** (내 가격대 맞나? — 청담 P30~P75 검증 핵심)
...
- `50-resources/data/zone-cache/{홍대입구,한남동,성수동,압구정동}.json` — 시연 캐시 (압구정동 본인 결 가까움)
- `.claude/skills/zone-diagnosis/SKILL.md` · `entry-verdict/SKILL.md`

## Day 2 도구 제작 후보 (방향 예시)

- 청담 한식 다이닝 예약·룸 관리 대시보드
- 객단가 분포·시간대별 회전율 분석
- VIP 단골 관리 (재방문·기념일)
- 다매장(본점+분점) 통합 운영 모니터링
